...
//!
//! Progress is logged to stderr. To see it: cargo test test_linear_regression -- --nocapture
...
fn make_dataset(backend: &Arc<CpuBackend>) -> Result<InMemoryDataset, dl_core::TensorError> {
    ...
    for _ in 0..N_SAMPLES {
        ...
        // Shape [2] / [1] so stack yields [B, 2] and [B, 1] for batch matmul.
...
/// One training run: assert loss decreases and learned weights match true [2, 3], bias 1.
#[test]
fn test_linear_regression() {
    ...
    for epoch in 0..EPOCHS {
        ...
        if epoch % 20 == 0 || epoch == EPOCHS - 1 {
            eprintln!("  epoch {}/{}  avg_loss={:.6}", epoch + 1, EPOCHS, avg_loss);
        }
    }

    let init = initial_loss.unwrap();
    let fin = final_loss.unwrap();
    eprintln!("  loss: initial={:.6} -> final={:.6} (epochs={})", init, fin, EPOCHS);
    assert!(fin < init, "loss should decrease: initial {} final {} (after {} epochs)", init, fin, EPOCHS);

    let w = trainer.model.weight.data();
    let b = trainer.model.bias.data();
    let wd = w.data();
    let bd = b.data();
    eprintln!("  weight=[{:.4}, {:.4}] (true [{}, {}])  bias={:.4} (true {})", wd[0], wd[1], TRUE_W[0], TRUE_W[1], bd[0], TRUE_B);
    assert_eq!(wd.len(), 2);
    assert_eq!(bd.len(), 1);
